...
(function($) {
  ...
  $.fn.tagEditor = function(options, val, blur) {
      ...
      $(document).off('keydown.tag-editor').on('keydown.tag-editor', function(e) {
        if (e.which == 8 || e.which == 46 || e.ctrlKey && e.which == 88) {
          try {
            ...
            var el = document.activeElement.tagName == 'BODY'
    ...
    return selector.each(function() {
      var el = $(this);
      var tag_list = []; // cache current tags
      ...
      function update_globals(init) {
        ...
        if (!init && old_tags != tag_list.toString()) {
      ...
      ed.on('click', '.tag-editor-delete', function(e) {
        ...
        var tag = li.find('.tag-editor-tag');
      ...
      ed.on('click', '.tag-editor-tag', function(e) {
        ...
        if (!$(this).hasClass('active')) {
          ...
          var input = $(this).html('<input type="text" maxlength="' + o.maxLength + '" value="' + escape(tag) + '">').addClass('active').find('input');
      ...
      function split_cleanup(input) {
        ...
        var sub_tags = input.val().replace(/ +/, ' ').split(o.dregex);
        var old_tag = input.data('old_tag');
        var old_tags = tag_list.slice(0);
        var exceeded = false;
        var cb_val; // copy tag_list
      ...
      ed.on('blur', 'input', function(e) {
        ...
        var input = $(this);
        var old_tag = input.data('old_tag');
        var tag = $.trim(input.val().replace(/ +/, ' ').replace(o.dregex, o.delimiter[0]));
        if (!tag) {
      ...
      ed.on('keydown', 'input', function(e) {
        ...
        if ((e.which == 37 || !o.autocomplete && e.which == 38) && !$t.caret() || e.which == 8 && !$t.val()) {
          ...
          return false;
        } else if (e.which == 36 && !$t.caret()) { // pos1
          ed.find('.tag-editor-tag').first().click();
        } else if (e.which == 35 && $t.caret() == $t.val().length) { // end
